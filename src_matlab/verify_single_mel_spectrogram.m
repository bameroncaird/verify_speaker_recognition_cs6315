function metric_struct = verify_single_mel_spectrogram(nnv_net, eps, line_number, plot)
    ...
    t = tic;
    [mel_spec, target, file_path] = read_data_line(line_number, 'libri100', 'train', 16000);
    mel_spec = single(mel_spec);
    %mel_spec = zeros([16 16 1]);

    metric_struct.target = target;
    metric_struct.file_path = file_path;

    % pass original input through the network (NNV network)
    pred_labels = nnv_net.evaluate(mel_spec);
    ...
    ones_ = ones(size(mel_spec), 'single'); % waveform is a single (float32)
    ...
    % we still normalized it in this range for the spectrograms
    % lb => Lower Bound
    % ub => Upper Bound
    lb_min = -128 * ones(size(mel_spec));
    ub_max = 127 * ones(size(mel_spec));
    lb_clip = max((mel_spec - disturbance), lb_min);
    ub_clip = min((mel_spec + disturbance), ub_max);

    % use ImageStar for 2d input
    lb_clip = lb_clip';
    ub_clip = ub_clip';
